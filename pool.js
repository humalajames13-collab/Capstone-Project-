...
function initPockets() {
    const offset = 4;
    pockets = [
        { x: offset, y: offset }, { x: canvas.width / 2, y: 0 }, { x: canvas.width - offset, y: offset },
        { x: offset, y: canvas.height - offset }, { x: canvas.width / 2, y: canvas.height }, { x: canvas.width - offset, y: canvas.height - offset }
    ];
}
    ...
    for (let i = 0; i < balls.length; i++) {
        for (let j = i + 1; j < balls.length; j++) {
            ...
            if (dist < BALL_RADIUS * 2) {
                ...
                if (p > 0) {
                    b1.vx -= nx * p * ELASTICITY; b1.vy -= ny * p * ELASTICITY;
                    b2.vx += nx * p * ELASTICITY; b2.vy += ny * p * ELASTICITY;
                }
            }
        }
    }
...
function evaluateTurnRules() {
    ...
    if (cueScratched) {
        ...
        cueBall.x = canvas.width * 0.25; cueBall.y = canvas.height / 2;
...
function executeBotTurn() {
    ...
    if (botDifficulty !== "easy" && chosenTarget) {
        let bestTarget = chosenTarget; let minDistance = Infinity;
        legalTargets.forEach(t => {
            pockets.forEach(p => {
                let dist = Math.hypot(p.x - t.x, p.y - t.y);
                if (dist < minDistance) { minDistance = dist; bestTarget = t; targetPocket = p; }
            });
        });
...
function handlePointerMove(clientX, clientY) {
    ...
    let cueBall = balls[0]; let rect = canvas.getBoundingClientRect();
    ...
    if (!cueStick.isDragging) {
        ...
        let dx = clientX - cueStick.dragStartX; let dy = clientY - cueStick.dragStartY;
        let pullDistance = Math.hypot(dx, dy);
        cueStick.power = Math.min(cueStick.maxPower, Math.max(0, pullDistance / 7));
        cueStick.angle = Math.atan2(dy, dx) + Math.PI; 
    }
}
...
canvas.addEventListener("mousedown", (e) => {
    if (isMoving || gameOver || (gameMode === "bot" && currentPlayer === 2)) return;
    if (e.button === 0) {
        cueStick.isDragging = true; cueStick.dragStartX = e.clientX; cueStick.dragStartY = e.clientY; cueStick.power = 0;
    }
});
...
window.addEventListener("mouseup", (e) => {
    ...
    if (cueStick.power > 1.5) {
        ...
        isMoving = true;
        if (currentPlayer === 1) { totalShotsTaken++; }
    }
    cueStick.power = 0;
});
...
function gameLoop() {
    ctx.fillStyle = "#0f766e"; ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.lineWidth = 10; ctx.strokeStyle = "#115e59"; ctx.strokeRect(0, 0, canvas.width, canvas.height);

    pockets.forEach(p => {
        ctx.beginPath(); ctx.arc(p.x, p.y, POCKET_RADIUS, 0, Math.PI * 2); ctx.fillStyle = "#022c22"; ctx.fill();
        ctx.lineWidth = 3; ctx.strokeStyle = "#042f2e"; ctx.stroke();
    });
    ...
    if (isMoving && !currentlyMoving) { isMoving = false; evaluateTurnRules(); }
    ...
    if (!isMoving && !gameOver && !(gameMode === "bot" && currentPlayer === 2)) {
        ...
        ctx.beginPath(); ctx.moveTo(stickStartX, stickStartY); ctx.lineTo(stickEndX, stickEndY);
        ctx.lineWidth = 5; ctx.strokeStyle = `rgb(${220 - cueStick.power * 3}, 155, 95)`; ctx.stroke();
        
        ctx.beginPath(); ctx.moveTo(stickStartX, stickStartY);
        ctx.lineTo(cueBall.x + Math.cos(cueStick.angle) * (gapOffset + 10), cueBall.y + Math.sin(cueStick.angle) * (gapOffset + 10));
        ctx.lineWidth = 5; ctx.strokeStyle = "#f8fafc"; ctx.stroke();
    }
...
difficultySelect.addEventListener("change", (e) => { botDifficulty = e.target.value; });
restartBtn.addEventListener("click", initGameSetup);

function initGameSetup() { initPockets(); initBalls(); }
